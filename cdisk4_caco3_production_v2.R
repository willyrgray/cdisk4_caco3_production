...
#seasonal bias satellite PIC and chlor_a
#pic for coccos
...
seasonal_bias_pic<- satellite_pic_aug2017/satellite_pic_annual_mean #ratio of pic Aug 2017 compared to annual climatology

#chlor_a for forams
satellite_chlor_a<- read.csv('seawifs_chlor_a_monthly_clim.csv')
satellite_chlor_a2017<- read.csv('seawifs_chlor_a_2017.csv')

satellite_chlor_a_annual_mean<- as.numeric(colMeans(satellite_chlor_a, na.rm=TRUE))
seasonal_bias_chlor_a<- as.numeric(satellite_chlor_a2017[8,])/satellite_chlor_a_annual_mean #ratio of chlorophyll Aug 2017 compared to annual climatology
...
for(i in 1:mcmc_iterations){
	...
	sbiasi_pic<- seasonal_bias_pic
	sbiasi_chlor_a<- seasonal_bias_chlor_a
	sbiasi_zoo<- seasonal_bias_zoo
		
	p1[i,]<- pi/sbiasi_zoo
	h1[i,]<- hi/sbiasi_zoo
	f1[i,]<- fi/sbiasi_chlor_a
	c1[i,]<- ci/sbiasi_pic
		
	ca1[i,]<- fi/seasonal_bias_chlor_a + ci/sbiasi_pic
	a1[i,]<- (pi + hi)/sbiasi_zoo
	s1[i,]<- fi/seasonal_bias_chlor_a + ci/sbiasi_pic + (pi + hi)/sbiasi_zoo
}
